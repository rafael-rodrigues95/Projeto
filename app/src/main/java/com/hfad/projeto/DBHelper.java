...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public int Utilizador_Login(String username, String pass) {
        ...
        Cursor c = db.rawQuery("SELECT * FROM utilizador WHERE username = ? AND pass = ?", new String[]{
                username, pass
        });
        c.moveToFirst();
        if (c.getCount() == 1) {
    ...
    public List<Utilizador> Utilizador_SelectAll() {
        ...
        if (c.getCount() > 0) {
            do {
                ...
                String username = c.getString(c.getColumnIndex("id"));
                String pass = c.getString(c.getColumnIndex("id"));
